# https://github.com/arminmokri/v2ray2json 
# make some customization : add listen_port arg , change inbound from socks to http
import json
import base64
import argparse
import re
...
def get_inbound(listen_port=1080):
    inbound = InboundBean(
        tag = "in_proxy",
        port = listen_port,
        protocol = HTTP,
...
def generateConfig(listen_port ,config: str, dns_list = ["8.8.8.8"]):
    ...
    if protocol == EConfigType.VMESS.protocolName:
        ...
        v2rayConfig = V2rayConfig(
            ...
            log = get_log(),
            inbounds = [get_inbound(listen_port)],
